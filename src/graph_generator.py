...
class GraphGenerator:
    def __init__(
        self, start_urls: list[str], allowed_domains: list[str], max_pages: int = 10
    ):
    ...
    def crawl(self, start_url: str, max_pages: int, delay: float = 1) -> None:
        """
        ...
        while queue and len(visited) < max_pages:
            ...
            extensions_to_filter: list[str] = [".jpg"]
    ...
    def extract_links_from_url(self, url: str) -> pd.DataFrame:
        try:
            headers = {
            ...
            return pd.DataFrame(
                columns=["link_text", "url", "link_type", "file_extension"]
            )
        ...
        df = pd.DataFrame({"url": link_urls, "file_extension": file_extensions})
    ...
    def analyze_graph(self) -> dict[str, Any]:
        ...
        stats: dict[str, Any] = {
            ...
            "Average out-degree": (
                sum(dict(self.G.out_degree()).values()) / len(self.G.nodes())
                if self.G.nodes()
                else 0
            ),
            "Average in-degree": (
                sum(dict(self.G.in_degree()).values()) / len(self.G.nodes())
                if self.G.nodes()
                else 0
            ),
        }
        ...
        if len(self.G.nodes()) > 0:
            ...
            top_inbound = sorted(in_degrees.items(), key=lambda x: x[1], reverse=True)[
                :5
            ]
    ...
    def get_visualization(self) -> None:
        ...
        node_trace = go.Scatter(
            ...
            hoverinfo="text",
            text=labels,
            marker=dict(showscale=False, color="#1f77b4", size=10, line_width=2),
        )
    ...
    def graph_to_json(self, output_file: str) -> None:
